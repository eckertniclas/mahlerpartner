...
;// ./src/javascripts/global/Parallax.js
...
;// ./src/javascripts/global/RightDrawer.js
...
let RightDrawer = (_dec = vue_class_component_esm({
  ...
  close() {
    ...
    // Emit an event to allow other components to update their state
    window.eventBus.emit('drawer:closed');
  }
  open() {
    ...
    // Emit an event to allow other components to update their state
    window.eventBus.emit('drawer:opened');
  }
      ...
      let resultWithPrice = result.map(item => {
        ...
        if (temp.price) {
          ...
          if (currencyCode && showCurrencyCode == 'true') {
            ...
            temp.b2bPrice = `${this.theme.formatMoney((temp_price/1.19) * 100)} ${currencyCode}`;
          } else {
            temp.price = this.theme.formatMoney(temp_price);
            temp.b2bPrice = this.theme.formatMoney((temp_price/1.19) * 100);
          }
        }
        return temp;
      });
